...
acelera t m j@(Jogador a b v c d) = novaGravidade (Jogador a b (novaVelocidade t m j) c d) t 
...
fromAr t m j@(Jogador p d v c (Ar alt inc g)) | (intersetam retaDoJogador (retaVerticalPeca j)) = (Jogador p limSup v c (Ar altF inc g))
                                              | otherwise      = if (not(intersetam retaDaPeca  retaDoJogador)) then Jogador p (posx posFinal) v c (Ar (posy posFinal) inc g)
                                              ...
                                              where (x,y)         = coordenadaJogador j
                                                    ...
                                                    pecaAtual     = encontraPosicaoMatriz (x,y) m
                                                    incPeca       = inclinacaoPeca (pecaAtual)
                                                    retaDoJogador = ((Cartesiano d alt), posFinal)
                                                    retaDaPeca    = retaPeca d (pecaAtual)
                                                    posFinal      = somaVetores (Cartesiano d alt) (multiplicaVetor t (somaVetores (Polar v inc) (Polar g (normalizaAngulo (-90)))))
